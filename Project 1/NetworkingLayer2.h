...
/*
 ...
 A chunk is defined as a sequence of bytes whose length is no greater than MAX_CHUNK_SIZE bytes.
 Each of the bytes in a chunk can have any value, including binary data and the '\0' character.
 The only restriction is that there are no more than MAX_CHUNK_SIZE bytes.
